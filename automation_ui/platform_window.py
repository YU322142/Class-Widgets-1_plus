...
def _get_macos_snapshot() -> ForegroundWindowSnapshot:
    ...
    try:
        ...
        # macOS 没有 Win32 类名概念，用 bundle id 近似
...
def _parse_net_wm_name(text: str) -> str:
    # 例：_NET_WM_NAME(UTF8_STRING) = "Some Title"
    m = re.findall(r'"([^"]*)"', text)
    if m:
        return m[0]
    # fallback
...
def _get_linux_snapshot() -> ForegroundWindowSnapshot:
    ...
    wm_class = _run_cmd(["xprop", "-id", win_id, "WM_CLASS"])
    class_name = _parse_wm_class(wm_class)
